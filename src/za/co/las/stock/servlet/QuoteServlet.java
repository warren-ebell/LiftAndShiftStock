...
public class QuoteServlet extends HttpServlet{
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if (serverMethod.equalsIgnoreCase(StockConstants.ACCEPT_QUOTE)) {
			...
			outputMessage = quotationService.acceptQuote(quotationId);
		}	
		if (serverMethod.equalsIgnoreCase(StockConstants.RETURN_STOCK)) {
			int quotationId = Integer.parseInt(req.getParameter("quotationId"));
			int result = quotationService.returnStock(quotationId);
			if (result == 1)
				outputMessage = outputMessage + "{'result':'1','message':''}";
			else
				outputMessage = outputMessage + "{'result':'0','message':'Stock update failed.'}";
		...
		if (serverMethod.equalsIgnoreCase(StockConstants.SEND_EMAIL)) {
			...
			String bodyText = "Dear "+quote.getCustomer().getAttention()+"\n"
					+ "\n"
					+ "Please find attached the quotation requested. Click this link to accept the quote ("+StockConstants.SERVER_URL+"/LiftAndShiftStock/quote?serverMethod=acceptQuote&quotationId="+quotationId+"), or comntact the sales team.\n"
					+ "Please do not reply to the email address this mail comes from, but rather the email address below.\n"
					+ "\n"
					+ "Kind Regards\n"
					+ user.getDisplayName()+"\n"

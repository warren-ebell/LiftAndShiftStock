...
public class QuotationDAO extends AbstractDAO {
	...
	public int insertQuotation(int customerId, ArrayList<String> stockItemIds, ArrayList<String> optionalExtraItemIds, String notes, String delivery, String installation, String warranty,String variation, String validity, String date, int userId, String serialNumber, double pricing) {
		Connection connection = getConnection();
		...
		try {
			...
			statement1 = connection.prepareStatement("insert into las_stock.quotation (customer_id, notes, delivery, installation, warranty, variation, validity, quotation_date, user_id) values (?,?,?,?,?,?,?,?,?)", Statement.RETURN_GENERATED_KEYS);
	...
	public int updateQuotationStock(int quotationId, int status) {
		...
		try {
			...
			statement = connection.prepareStatement("select qli.stock_id, qli.serial_number from las_stock.quotation q, las_stock.quotation_line_item qli where q.quotation_id = ? and q.quotation_id = qli.quotation_id");
	...
	public Quotation getQuotationForQuotationId(int quotationId) {
		...
		try {
			//1. get the quotation information for this quote...
			statement = connection.prepareStatement("select quotation_id, notes, delivery, installation, warranty, variation, validity, quotation_date, user_id from las_stock.quotation where quotation_id = ?");
			statement.setInt(1, quotationId);
	...
	public ArrayList<MiniQuote> getquoteHistoryList() {
		...
		try {
			statement = connection.prepareStatement("select c.customer_name, q.quotation_id, q.quotation_date from las_stock.customer c, las_stock.quotation q where q.customer_id = c.customer_id");
	...
	public Defaults getDefaults() {
		...
		try {
			statement = connection.prepareStatement("select notes, delivery, installtion, warranty, variation, validity from las_stock.defaults");
			...
			while (resultSet.next()) {
				...
				defaults.setInstallation(resultSet.getString("installtion"));
